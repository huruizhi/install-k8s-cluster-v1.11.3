...
```bash
# https://pan.baidu.com/s/1PduVTuR7RJbzR0Q76ALJ4w 百度网盘
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh root@${node_ip} "mkdir -p /application/etcd && ln -s /application/etcd /var/lib/ && chown -R k8s /application/etcd" 
...
```bash
>>> 192.168.0.162
https://192.168.0.162:2379 is healthy: successfully committed proposal: took = 1.362704ms
>>> 192.168.0.165
https://192.168.0.165:2379 is healthy: successfully committed proposal: took = 2.04951ms
>>> 192.168.0.166
https://192.168.0.166:2379 is healthy: successfully committed proposal: took = 2.4527ms
```
