...
for node_ip in ${NODE_IPS[@]}
  do
    ...
    ssh root@${node_ip} "mkdir -p /etc/haproxy"
...
listen kube-master
    ...
    option tcplog
    balance source
    server 192.168.0.162 192.168.0.162:6443 check inter 2000 fall 2 rise 2 weight 1
    server 192.168.0.165 192.168.0.165:6443 check inter 2000 fall 2 rise 2 weight 1
    server 192.168.0.166 192.168.0.166:6443 check inter 2000 fall 2 rise 2 weight 1
EOF
```
...
+ master: 192.168.0.162
+ backup：192.168.0.165、192.168.0.166
...
```bash
scp keepalived-master.conf root@192.168.0.162:/etc/keepalived/keepalived.conf
```

下发 backup 配置文件：

```bash
scp keepalived-backup.conf root@192.168.0.165:/etc/keepalived/keepalived.conf
scp keepalived-backup.conf root@192.168.0.166:/etc/keepalived/keepalived.conf
```
